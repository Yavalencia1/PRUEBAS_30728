...
class Usuario(Base):
    ...
    primer_ingreso: Mapped[bool] = mapped_column(
        Boolean,
        nullable=False,
        default=True,
        server_default="true"
    ...
    recorridos_dueno = relationship(
        "Recorrido",
        back_populates="dueno",
        cascade="all, delete-orphan"
    )

    alumnos_padre = relationship(
        "Alumno",
        back_populates="padre",
        foreign_keys="Alumno.padre_id"
    )

    sesiones_conductor = relationship(
        "SesionRuta",
        back_populates="conductor",
        foreign_keys="SesionRuta.conductor_id"
    )

    pagos_padre = relationship(
        "Pago",
        back_populates="padre",
        foreign_keys="Pago.padre_id"
    )
