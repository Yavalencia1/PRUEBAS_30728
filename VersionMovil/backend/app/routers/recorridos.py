...
from app.schemas.recorrido import RecorridoActualizar, RecorridoCrear
...
@router.patch("/{recorrido_id}", response_model=dict)
async def actualizar_recorrido(
	...
	if usuario.rol not in (RolUsuario.admin, RolUsuario.dueno):
		raise HTTPException(
			status_code=status.HTTP_403_FORBIDDEN,
			detail="No tienes permisos para modificar recorridos",
		)
	...
	if usuario.rol == RolUsuario.dueno and recorrido.dueno_id != usuario.id:
		raise HTTPException(
			status_code=status.HTTP_403_FORBIDDEN,
			detail="No tienes permisos para modificar este recorrido",
		)
	...
	recorrido_actualizado = await _obtener_recorrido_o_404(db, recorrido_id)
	accion = "desactivado" if datos.activo is False else "actualizado"
	return _respuesta_estandarizada(
		_serializar_recorrido(recorrido_actualizado),
		f"Recorrido {accion} correctamente",
	)
...
async def eliminar_recorrido(
	...
	"""Elimina un recorrido físicamente (cascada a rutas, paradas, sesiones, alumnos).
	Solo administradores pueden ejecutar esta acción.
	"""
	if usuario.rol != RolUsuario.admin:
		raise HTTPException(
			status_code=status.HTTP_403_FORBIDDEN,
			detail="Solo administradores pueden eliminar recorridos",
		)
	...
	try:
		...
		raise HTTPException(
			status_code=status.HTTP_400_BAD_REQUEST,
			detail="No se pudo eliminar el recorrido",
		) from error
