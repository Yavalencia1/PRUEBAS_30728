...
async def eliminar_alumno(
	...
	"""Elimina un alumno físicamente (cascada a asistencias y pagos).
	Solo administradores pueden ejecutar esta acción.
	"""
	if usuario.rol != RolUsuario.admin:
		raise HTTPException(
			status_code=status.HTTP_403_FORBIDDEN,
			detail="Solo administradores pueden eliminar alumnos",
		)

	alumno = await _obtener_alumno_o_404(db, alumno_id)
